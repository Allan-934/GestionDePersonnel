...
class C_ajouterEmploye
{
    ...
    public function action_ajout($matricule, $nom, $prenom, $service)
    {
        ...
        if (is_null($this->modeleEmploye->GetEmploye($matricule))) //le matricule n'existe pas
        {
            ...
            if (is_null($employe)) {
                $this->data['leMessage'] = "L'ajout a échoué pour une raison
                          indéterminée.";
            } else {
                $this->data['leMessage'] = $employe->GetNom() . " " . $employe->GetPrenom()
                    . " a été ajouté.";
            }
        } else {
            $this->data['leMessage'] = "Le matricule " . $matricule
                . " existe déjà, ajout annulé.";
        }
